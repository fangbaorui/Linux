  ...
  class Server
  {
    ...
    private:
      static void Upload(const httplib::Request &req,httplib::Response &rsp)
      {
        ...
        auto ret = req.has_file("file");//判断有没有对应的name的文件上传信息
        if(ret == false)
        {
      ...
      static void List(const httplib::Request &req,httplib::Response &rsp)
      {
        ...
        ss<<"<hr /></body></html>";
        rsp.body = ss.str();
        rsp.set_header("Content-Type","test/html");
        return;
      }
...
#define PACK_PATH "./packdir/"
  class FileManager{
    private:
      ScanDir _scan;
      time_t _hot_time = 60;//示多少秒没被访问就算是非热点文件
      ...
      bool Start()
      {
        ...
        while(1)
        {
          ...
          for(auto& file : arry)
          {
            ...
            std::cout<<file<<":"<<atime<<"-"<<ctime<<std::endl;
