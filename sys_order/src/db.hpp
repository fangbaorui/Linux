...
namespace order_sys{
  ...
  static MYSQL* MysqlInit(){
    ...
    //连接服务器
    //mysql_real_connect(句柄，服务器IP，用户名，密数据库名称，端口，套接字文件，客户端标志)
    if(mysql_real_connect(mysql,MYSQL_SERVER,MYSQL_USER,MYSQL_PASSDW,MYSQL_DBNAME,0,NULL,0)==NULL)
    {  
...
#define DISH_SELECTALL "select * from tb_dish;"
           ...
           //执行语句和保存结果集需要加锁（接口里面是原子操作，接口与接口之间不是原子操作）
          _mutex.lock();
